...
from .compute_isochrones import tiempo_a_salud, _vulnerabilidad, calcular_rutas
from .seasonal import penalizar_aristas
from .compute_scores import calcular_ranking
from .export import export_acceso, export_tramos, export_sqlite, export_hospitales, export_rutas
...
def main():
    ...
    # ------------------------------------------------------------------
    # [1/7] Grafo OSM + puntos enganchados
    # ------------------------------------------------------------------
    print("[1/7] Grafo OSM y carga de puntos...")
    ...
    # [2/7] Acceso seco
    # ------------------------------------------------------------------
    print("[2/7] Calculando acceso seco...")
    ...
    # [3/7] Acceso lluvias
    # ------------------------------------------------------------------
    print("[3/7] Calculando acceso lluvias...")
    ...
    # ------------------------------------------------------------------
    # [4/7] Rutas de acceso (trayectorias de red hospital -> CCPP)
    # ------------------------------------------------------------------
    print("[4/7] Reconstruyendo rutas de acceso...")
    gdf_rutas_seco    = calcular_rutas(G,        nodos_salud, salud_eng,
                                       df_seco,   "en_brecha")
    gdf_rutas_lluvias = calcular_rutas(G_lluvia,  nodos_salud, salud_eng,
                                       df_lluvias, "en_brecha_lluvias")
    print(f"      Rutas seco: {len(gdf_rutas_seco)} | "
          f"Rutas lluvias: {len(gdf_rutas_lluvias)}")

    # ------------------------------------------------------------------
    # [5/7] Ranking de tramos
    # ------------------------------------------------------------------
    print("[5/7] Calculando ranking de tramos...")
    ...
    # ------------------------------------------------------------------
    # [6/7] Exportar a backend/data/
    # ------------------------------------------------------------------
    print("[6/7] Exportando a backend/data/...")
    ...
    export_rutas(gdf_rutas_seco,    "seco")
    export_rutas(gdf_rutas_lluvias, "lluvias")

    # ------------------------------------------------------------------
    # [7/7] Verificacion
    # ------------------------------------------------------------------
    print("[7/7] Verificando archivos en backend/data/...")
    esperados = [
        "acceso_seco.geojson", "acceso_lluvias.geojson",
        "tramos_candidatos.geojson", "app.db",
        "hospitales.geojson",
        "rutas_seco.geojson", "rutas_lluvias.geojson",
    ]
